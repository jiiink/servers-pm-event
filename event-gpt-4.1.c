...
static void resume_event(struct mproc *rmp) {
    ...
    for (i = rmp->mp_eventsub; i < nsubs; i++) {
        ...
        if (subs[i].mask & event) {
            ...
            if (subs[i].waiting >= NR_PROCS) {
                panic("too many waiting procs: %u", subs[i].waiting);
            }
            subs[i].waiting++;
            rmp->mp_eventsub++;
            return;
...
int do_proceventmask(void)
{
    ...
    if (nsubs >= __arraycount(subs)) {
        (void)printf("PM: too many process event subscribers!\n");
        return ENOMEM;
    }
...
int do_proc_event_reply(void)
{
    ...
    if (subs[i].waiting <= 0) {
        printf("PM: proc event reply from %d for %d, no waiting subscribers\n", who_e, endpt);
        return SUSPEND;
    }
    subs[i].waiting--;
...
void publish_event(struct mproc *rmp)
{
    ...
    if (nested != 0) return;
    if ((rmp->mp_flags & (IN_USE | EVENT_CALL)) != IN_USE) return;
    if (rmp->mp_eventsub != NO_EVENTSUB) return;
